...
class Dir implements IDir
{
    ...
    public function ensureExists(int $permissions=null): IDir
    {
        if (!is_dir($this->path)) {
            if (file_exists($this->path)) {
                throw Df\Error::EIo('Dir destination exists as file', null, $this);
            }
            ...
            if (!mkdir($this->path, $permissions, true)) {
                throw Df\Error::EIo('Unable to mkdir', null, $this);
            }
        } else {
            if ($permissions !== null) {
    ...
    {
        if (!$this->exists()) {
            throw Df\Error::ENotFound('Cannot set permissions, dir does not exist', null, $this);
        }
    ...
    {
        if (!$this->exists()) {
            throw Df\Error::ENotFound('Cannot set owner, dir does not exist', null, $this);
        }
    ...
    {
        if (!$this->exists()) {
            throw Df\Error::ENotFound('Cannot set group, dir does not exist', null, $this);
        }
    ...
    {
        if (file_exists($path)) {
            throw Df\Error::EIo('Destination dir already exists', null, $this);
        }
    ...
    {
        if (!$this->exists()) {
            throw Df\Error::ENotFound('Source dir does not exist', null, $this);
        }
        ...
        if ($newName == '' || $newName === '..' || $newName === '.' || strstr($newName, '/')) {
            throw Df\Error::EInvalidArgument('New dir name is invalid: '.$name, null, $this);
        }
        ...
        if (!rename($this->path, $destination)) {
            throw Df\Error::EIo('Unable to rename dir', null, $this);
        }
