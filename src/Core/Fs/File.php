...
class File extends Stream implements IFile
{
    ...
    public function open(string $mode): IFile
    {
        ...
        if (!$this->resource = fopen($this->path, $mode)) {
            throw Df\Error::EIo('Unable to open file', null, $this);
        }
    ...
    {
        if (!$this->exists()) {
            throw Df\Error::ENotFound('Cannot set permissions, file does not exist', null, $this);
        }
    ...
    {
        if (!$this->exists()) {
            throw Df\Error::ENotFound('Cannot set owner, file does not exist', null, $this);
        }
    ...
    public function unlock(): IFile
    {
        ...
        if (!flock($this->resource, LOCK_UN)) {
            throw Df\Error::EIo('Unable to unlock file', null, $this);
        }
    ...
    {
        if (!$this->exists()) {
            throw Df\Error::ENotFound('Source file does not exist', null, $this);
        }
        ...
        if ($newName == '' || $newName === '..' || $newName === '.' || strstr($newName, '/')) {
            throw Df\Error::EInvalidArgument('New file name is invalid: '.$name, null, $this);
        }
        ...
        if (file_exists($destination)) {
            throw Df\Error::EIo('Destination file already exists', null, $destination);
        }

        if (!rename($this->path, $destination)) {
            throw Df\Error::EIo('Unable to rename file', null, $this);
        }
    ...
    {
        if ($this->resource === null) {
            throw Df\Error::EIo('Cannot seek file, file not open', null, $this);
        }

        if (0 !== fseek($this->resource, $offset, $whence)) {
            throw Df\Error::EIo('Failed to seek file', null, $this);
        }
    ...
    {
        if ($this->resource === null) {
            throw Df\Error::EIo('Cannot ftell file, file not open', null, $this);
        }

        $output = ftell($this->resource);

        if ($output === false) {
            throw Df\Error::EIo('Failed to ftell file', null, $this);
        }
    ...
    {
        if ($this->resource === null) {
            throw Df\Error::EIo('Cannot flush file, file not open', null, $this);
        }

        $output = fflush($this->resource);

        if ($output === false) {
            throw Df\Error::EIo('Failed to flush file', null, $this);
        }
