...
class Text implements \IteratorAggregate, data\ICollection, \ArrayAccess, \Countable, lang\IPipe
{
     ...
     */
    public static function create(string $text='', string $encoding=null)
    {
     ...
     */
    public function append(string $string): Text
    {
        return new static($this->text.$string, $this->encoding);
    }

    /**
     * Add a string to the start
     */
    public function prepend(string $string): Text
    {
        return new static($string.$this->text, $this->encoding);
    }

    /**
     * Add string to start and end
     */
    public function surround(string $string): Text
    {
        return new static($string.$this->text.$string, $this->encoding);
    }
    ...
    public function matches(string $pattern, string $options='msr'): bool
    {
        ...
        $oldOptions = mb_regex_set_options();
    ...
    public function regexReplace(string $pattern, string $replacement, string $options='msr'): Text
    {
        ...
        $output = mb_ereg_replace($pattern, $replacement, $this->text, $options);
    ...
    public function regexSplit(string $pattern, int $limit=-1): array
    {
        return array_map(function ($part) {
            return new static($part, $this->encoding);
        }, mb_split($pattern, $this->text, $limit));
    }
    ...
    public function toAscii(string $language='en', bool $removeUnsupported=true): Text
    {
        ...
        foreach (static::ASCII_CHARS as $key => $value) {
            $output = str_replace($value, $key, $output);
        }
    ...
    public function alphaToNumeric(): ?int
    {
        ...
        for ($i = 0; $i < $length = mb_strlen($this->text); $i++) {
            $char = mb_substr($this->text, $i, 1, $this->encoding);
     ...
     */
    public function searchAll(string $pattern, int $limit=null, string $options='msr'): \Iterator
    {
        if ($pattern === '') {
            return;
        }
     ...
     * Iterate over all split instances
     */
    public function scan(string $pattern, int $limit=null, bool $yieldMatch=false, string $options='msr'): \Iterator
    {
        if ($pattern === '') {
            return;
        }
     ...
     * Iterate over lines
     */
    public function scanLines(): \Iterator
    {
        return $this->scan('[\r\n]{1,2}');
    }
     ...
     */
    public function scanWords(): \Iterator
    {
        return $this
            ->regexReplace('\p{P}', ' ')
            ->scan('[[:space:]]+');
